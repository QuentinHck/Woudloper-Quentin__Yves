...
function dropDownEffect(event) {
  ...
  if (event.target === $dropdownHamburger) {
    ...
    event.target === $signupDropdownWrapper ||
    event.target === $signupDropdownButton
  ) {
    openDropdown($signupDropdownButton, $signupFieldForm);
  } else if (
    event.target === $agendaButton ||
    event.target === $agendaDropdownButton ||
    event.target === $agendaDropdownAfter
  ) {
